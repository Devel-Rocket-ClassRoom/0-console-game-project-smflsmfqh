...
- **Engine**: 게임 루프, 렌더링, 입력 처리. 수정하지 않습니다. -> 필요시 수정 가능
...
```
    ...
    MyGame (여러분의 GameApp 서브클래스)
       │  MyGame has a SceneManager
       ▼
  SceneManager<TScene> // 씬을 변경 시켜주는 관리자 역할
       │
       ▼
    Scene (abstract)
       △
  ...
  GameObject (abstract)
       △
    ┌──┴──────┬─────────┐
  Player    Enemy    ... (여러분의 게임 오브젝트들) // GameObejct를 상속받아 오브젝트들 구현 -> 배열에 인스턴스 담아서 씬에서 활용

  ScreenBuffer ← GameApp이 소유, Scene에 전달 // 그리기 역할을 하는 클래스
  Input        ← 정적 클래스, 어디서든 접근 // 키보드 입력에 관한 클래스
```
